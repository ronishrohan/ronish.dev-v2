  ...
  return (
    <>
      <div className="hidden sm:block pointer-events-none">
        ...
        ></div>
        <div className="h-[100vh] w-screen -z-10 p-4 justify-end pointer-events-auto fixed bottom-0 flex flex-col pt-10 bg-black text-white">
          <motion.div
          ...
          <div className="size-full flex gap-4 my-4 items-end">
            <motion.div
              style={{
                y: `${100 - simple * 100}%`,
                height: `${Math.max(75, simple * 100)}%`,
                opacity: simple,
              }}
              className="overflow-hidden w-2/3 h-full gap-4 shrink-0 flex flex-col"
            >
              <div className="size-full flex gap-4">
                <ContactButton
                  description="I post my art here"
                  title="Instagram"
                ></ContactButton>
                <ContactButton
                  description="You can contact me here, always active :/"
                  title="Discord"
                ></ContactButton>
                <ContactButton
              ...
              <div className="size-full flex gap-4">
                <ContactButton
                  description="Send me mail or something idk"
                  title="Gmail"
                ></ContactButton>
                <ContactButton
                  description="Where I shitpost"
                  title="Reddit"
                ></ContactButton>
                <ContactButton
                  description="All my repositories are here, feel free to check them out"
                  title="Github"
                ></ContactButton>
              </div>
            </motion.div>
          ...
          <motion.div
            style={{ opacity: simple }}
            className="mt-auto w-full shrink-0 flex text-white/60 font-normal gap-2"
          >
      ...
      <div id="contact2" className="sm:hidden bg-black text-white flex flex-col w-full h-fit p-2">
        <div className="text-[10vw]">RONISH ROHAN</div>
...
const ContactButton = ({
  ...
  return (
    <button
      ...
      className="cursor-none size-full overflow-hidden border group hover:fill-black relative fill-white flex-col transition-colors border-white/40 rounded-lg flex items-start justify-start p-4 hover:bg-white hover:text-black"
    >
      {title}
